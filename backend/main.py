...
def get_document_by_uuid(uuid_value, path):
    ...
    document = mongo.db.documents.find_one({"uuid": uuid_value}, projection)
    if document:
        return document
    return None
...
def handle_update(current_user,data):
    ...
    mongo.db.documents.update_one({"uuid": doc_id}, {"$set": update_data})
...
def accept_invite():
    ...
    try:
        ...
        # Create the reference object for the user's createdDocuments
        document = mongo.db.documents.find_one({"uuid": doc_id})
        document_reference = {
            "title": document['title'],
            "url": document['url'],
